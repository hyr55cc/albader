...
function renderHijriCalendar() {
    ...
    for (var day = 1; day <= daysInHijriMonth; day++) {
        ...
        var dateStr = formatDate(gregDate);
    ...
    if (hijriCalendar) {
        ...
        var cells = hijriCalendar.querySelectorAll('.day-cell[data-date]');
...
function renderGregorianCalendar() {
    ...
    if (gregorianCalendar) {
        ...
        // إضافة event listeners
        var cells = gregorianCalendar.querySelectorAll('.day-cell[data-date]');
        for (var i = 0; i < cells.length; i++) {
            cells[i].addEventListener('click', function(e) {
                var date = this.getAttribute('data-date');
                selectDate(date);
